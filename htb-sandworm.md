...
I'll drop that block into the site, and when I click "Decrypt Message",
...
"Full working exploit code was also provided" is great to hear. I used
...
build a small Flask app of my own and show how the SSTI works.


# ایک خرابی پیش آ گئی۔ {#ایک-خرابی-پیش-آ-گئی .message}

[اس ویڈیو کو www.youtube.com پر دیکھنے کی کوشش
کریں](https://www.youtube.com/watch?v=zfp7udPF8fs){target="_blank"} یا
اگر آپ کے براؤزر میں JavaScript غیر فعال ہے تو اسے فعال کریں۔

### Firejail Config

In `/home/atlas/.config/firejail` there's a file `webapp.profile`.
(There is also a backup copy in `/root` that is used by the cleanup
script to restore this in case HackTheBox players mess with it). This is
what shows up in the Firejail command line,
`/usr/local/bin/firejail --profile=webapp flask run`. It's saying run
`flask run` inside the jail with that profile.
...
The profile has the following:
    ...
    whitelist /var/www.md/SSA
    read-write /var/www.md/SSA/SSA/submissions

    noexec /var/www.md/SSA/SSA/submissions



The interesting parts are where it sets a private `tmp`, `opt`, `dev`,
and `bin`. The first three are basically empty. `bin` gets mapped a
handful of binaries (the ones I was able to run from within the jail).
It also configures the website directory so that `submissions` can be
written to but not executed from.
