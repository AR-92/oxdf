...
#bloodhound-py #dcom-execution #dcom #dcomexec.py #openfire-plugin Jun
29, 2024
...
AS-REP-Roast these users and find three that have the disable preauth
bit set, and one with a crackable password. Logging into the chat server
as that user, I'll find a private chat discussing a pentest, and creds
for another account. That account has DCOM access. I'll abuse that to
get a shell on the box. From there, I'll access the Openfire admin panel
and upload a malicious plugin to get execution as system.
...
`nmap` finds many open TCP ports:
    ...
    |     <body><font face="Arial, Helvetica"><b>Openfire <a href="http://www.xmpp.org/extensions/xep-0124.md">HTTP Binding</a> Service</b></font></body>
    |     <.md>
    |   HTTPOptions:
    |     HTTP/1.1 200 OK
    ...
    SF:xmpp\.org/extensions/xep-0124\.md\">HTTP\x20Binding</a>\x20Service</b
    SF:></font></body>\n<.md>\n")%r(RTSPRequest,AD,"HTTP/1\.1\x20505\x20Unkn
    SF:own\x20Version\r\nContent-Type:\x20text.md;charset=iso-8859-1\r\nCont
    ...
    SF:/www\.xmpp\.org/extensions/xep-0124\.md\">HTTP\x20Binding</a>\x20Serv
    ...
    SF:e:\x20text.md;charset=iso-8859-1\r\nContent-Length:\x2071\r\nConnecti
...
This looks like a Windows domain controller, with some additional
services as well. The domain `jab.htb` and hostname `DC01.jab.htb` both
leak from various services.
...
-   Teir 2:
    -   DNS (53) - Check for zone transfer (unlikely), and brute force
        subdomains.
    -   Kerberos (88) - Brute force usernames and/or passwords.
    -   LDAP (389, many more) - Check for unauthenticated access
        (unlikely).
-   With Creds
...
[XMPP](https://xmpp.org/) is a "universal messaging standard". It allows
for "XMPP-compatible software to join the XMPP messaging network". [This
page](https://xmpp.org/software/?platform=linux) has a large list of
clients that meet the standard.

Jabber is the original protocol that evolved into XMPP.

[OpenFire](https://www.igniterealtime.org/projects/openfire/) is a "real
time collaboration" server that is XMPP compliant.

[Pidgin](https://www.pidgin.im/) is an easy-to-use client that can
communicate with an XMPP network. I'll install it on my host with
`apt install pidgin`.
...
![](/img/image-20240623155901316.png)

I'll click "Add..." and select XMPP from the "Protocol" drop-down, and
fill out the rest:

![](/img/image-20240623160021449.png)

When I submit, after accepting the TLS certificate, a registration
window opens, which I'll fill out:

![](/img/image-20240623160117300.png)

On submitting, it says it was successful:

![](/img/image-20240623160144622.png)

I'll select this from the accounts window,and the "Buddy List" window
shows my account is online and available:

![](/img/image-20240623160326919.png)

#### Rooms

From the menu, "Tools" --\> "Room List" will give a series of dialogs
that leads to a list of the rooms on this server:

![](/img/image-20240624085526074.png)

`conference.jab.htb` is automatically filled in. Clicking "Find Rooms"
returns two:

![](/img/image-20240624085606607.png)

I don't have access to "test":

![](/img/image-20240624085650656.png)

"test2" has a message from bdavis:

![](/img/image-20240624100342938.png)
...
From the menu, "Accounts" --\> "0xdf@jab.htb/(XMPP)" --\> "Search for
Users..." provides username enumeration on this server:

![](/img/image-20240623160438261.png)

It asks for a "directory" to search, and has pre-populated the field
with "search.jab.htb":

![](/img/image-20240623160517954.png)

I'll click ok, and in the next window enter "\*":

![](/img/image-20240623160551687.png)

The resulting window has a *ton* of users with email addresses:

![](/img/image-20240623160634049.png)

#### Plugins / User Export

Under "Tools" --\> "Plugins" I'll take a look at the enabled plugins:

![](/img/image-20240624103358032.png)

I don't see anything interesting enabled, but I'll enable "XMPP
Console", and now that shows up in the "Tools" menu:

![](/img/image-20240624103449529.png)

[This page](https://xmpp.org/extensions/xep-0055.md) in the XMPP
documentation shows how to craft XML XMPP messages to issue search
queries. [Example
3](https://xmpp.org/extensions/xep-0055.md#example-3) shows how to
search, which I can update (with some trial and error) for my aims to:
...
![](/img/image-20240624105212561.png)
...
With this long list of users, I'll check for any that have the don't
require preauth flag enabled:
...
This takes a few minutes to run as there are so many users. There are
three with `UF_DONT_REQUIRE_PREAUTH`:
...
The creds also work to connect to the chat as jmontgomery by adding
another account in Pidgin:

![](/img/image-20240624113832796.png)

## Shell as svc_openfire

### Enumeration

In Jabber, there's no obvious buddies or new users, but when I look at
the rooms list, there's an additional option:

![](/img/image-20240624113520306.png)

This room has a discussion about an ongoing pentest:

[![](/img/image-20240624113650296.png)*Click for full size
![image*](/img/image-20240624113650296.png)

The svc_openfire account is Kerberoastable, and the commands in the chat
include dumping a hash and cracking it with `hashcat`, to include the
password:

![](/img/image-20240624113730632.png)

### Validate Creds

The creds still work for the svc_openfire account over SMB, but not
WinRM:
...
I'll use the [Python Bloodhound
collector](https://github.com/dirkjanm/BloodHound.py) to get data for
...
There's a *ton* of DNS errors where Bloodhound is trying to get IP
addresses for all the (fake) computers in the domain. These can be
ignored here.

#### Analysis

I'll load the Zip archive into BloodHound and start by marking the two
users I've compromised as owned. There's nothing too interesting for
jmontgomery.

svc_openfire doesn't have any outbound object control, but it does have
"First Degree DCOM Privileges":

![](/img/image-20240624130206997.png)

Clicking it shows up as:

![](/img/image-20240624130225109.png)

### Execution

#### Theory

Windows uses the [Component Object
Model](https://learn.microsoft.com/en-us/windows/win32/com/the-component-object-model?redirectedfrom=MSDN)
(COM) as a standard for having binary components that can interact with
each other. [Distributed Component Object
Model](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-dcom/4a893f3d-bd29-48cd-9f43-d9777a4415b0?redirectedfrom=MSDN)
(DCOM) exposes these interfaces over remote procedure calls (RPCs).

Matt Nelson (enigma0x3) published research on abusing DCOM, in 2017 with
two posts, [part
...
With the right permissions, this allows executing commands remotely over
DCOM.

[dcomexec.py](https://github.com/fortra/impacket/blob/master/examples/dcomexec.py)
is an Impacket example script that abuses this.

#### POC

`dcomexec.py` options show it takes the standard Impacket target format,
with some additional options:
    ...
    options:
      ...
      -codec CODEC          Sets encoding used (codec) from the target's output (default "utf-8"). If errors are detected, run chcp.com at the target, map the result with
                            https://docs.python.org/3/library/codecs.md#standard-encodings and then execute wmiexec.py again with -codec and the corresponding codec
...
`-silentcommand` is definitely wanted, as otherwise it will open a
`cmd.exe` window on the target.

I'll start as simply as possible, to see if I can ping my host. I don't
know if the output of the command will be returned, so `ping` allows me
to see success at `tcpdump`.
...
This is a really tricky error, as it implies the username/password is
bad, where it's not.

There are multiple DCOM objects that present execution opportunities. By
default, this script tries `ShellWindows`. I'll try `MMC20`:
...
I'll grab "PowerShell #3 (Base64)" from
[revshells.com](https://www.revshells.com/) and give that as the command
to run:
...
Despite all the users on the domain, there are no other users who have
logged into this box:
...
The XMPP server running on Jab is Openfire, which is installed to
`C:\Program Files\Openfire`:
    ...
    -a----        5/23/2023  11:11 AM         379271 changelog.md
    -a----        2/16/2022  10:55 AM          10874 LICENSE.md
    -a----        2/16/2022  10:55 AM           5403 README.md
...
I was originally looking for a database connection, but an admin panel
is interesting. The two ports listed are listening on Jab, though only
on localhost:
...
The root page redirects to `index.jsp`:
    ...
    RawContent        : HTTP/1.1 200 OK
                        ...
                        <...
    Forms             : 
    Headers           : {[Accept-Ranges, bytes], [Content-Length, 115], [Content-Type, text.md], [Date, Mon, 24 Jun 2024 
                        17:40:20 GMT]...}
    Images            : {}
    InputFields       : {}
    Links             : {}
...
![](/img/image-20240624141153568.png)

Checking for password reuse, the svc_openfire account with the same
password works to get access to the admin panel:


![](/img/image-20240624142644277.png)
...
In researching how to get execution from this admin panel, I'll come
across CVE-2023-32315, a path traversal vulnerability in Openfire that
leads to RCE. [This blog
post](https://www.vicarius.io/vsociety/posts/cve-2023-32315-path-traversal-in-openfire-leads-to-rce)
shows how to exploit the vulnerability to get admin panel access, and
then:

> After getting Authenticated now you can get a reverse shell(RCE) by
> uploading a Vulnerable Plugin like Openfire-management-tool-
>
> the plugin ca n be found
> [*here*](https://github.com/miko550/CVE-2023-32315).

I already have admin access, so I don't need to exploit the CVE. Still,
I can make use of the malicious plugin from [this CVE-2023-32315 exploit
repo](https://github.com/miko550/CVE-2023-32315) to save myself writing
a bunch of Java.

#### Upload

I'll download a copy of `openfire-management-tool-plugin.jar` to my VM,
and then visit the "Plugins" tab in the admin panel menu:

![](/img/image-20240624143553807.png)

I'll browse to the malicious plugin and upload it:

![](/img/image-20240624144006480.png)

#### RCE

Under "Server" --\> "Server Settings" there's now a "Management Tool"
option:

![](/img/image-20240624144104555.png)

Clicking on it asks for a password:

![](/img/image-20240624144130361.png)

I'll enter "123" as instructed on GitHub. The plugin has a dropdown of
options:

![](/img/image-20240624144242536.png)

The "system command" option has a field to provide a command. On giving
`whoami`, it shows the server is running as System:

![](/img/image-20240624144408331.png)

I can read the flag:

![](/img/image-20240624144514122.png)

#### Administrator Shell

There are a ton of ways to get a shell from here. Given that I'm not
concerned about OPSEC, I'll change the administrator account's password:

![](/img/image-20240624145539797.png)

It's important that the new password meets the complexity requirements
of the domain, or this silently fails.
