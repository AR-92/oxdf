...
#pspy #cron #oswe-like Aug 25, 2018
...
deserialization vulnerabilities in Node.js. Weather it's in struts, or
python's pickle, or in Node.js, deserialization of user input is almost
always a bad idea, and here's we'll show why. To escalate, we'll take
...
nmap doesn't tell us, but port 3000 is the default port for Node.js /
Express.
...
-   "X-Powered-By: Express" - this is in line with our suspicious about
    port 3000 being Express.
...
It is easy to make the page crash by messing with the parameters, giving
it non-string values, etc.
...
Next we'll create `exploit.js`, which will create the serialized payload
by putting some wrapping around the shell we just created:
...
This is the same format used in the blog post, though I added the
slicing in the last line to add the "()", where as he added it manually
to the output.
...
Finally, we'll base64 encode that to get to the format of the cookie.
I'll use `tail -n +2` to drop the first line:
...
Use burp to intercept a web request and add our modified cookie. One
submitting, the page returns:
...
Looking at the file modify times, it's clear that output.txt is very
recently updated. In fact, it's being updated every 5 minutes:
...
In the Documents directory next to user.txt, there's a script.py that
creates that output:
...
There are enough clues on this host that simple enumeration should tip
us off to the cron. But, in other cases, we won't be so lucky. This is a
cool chance to show off an awesome tool, `pspy`. I gave a quick overview
on how to install it in the [Aragog
write-up](/htb-aragog.md) . But let's run it here just for
kicks.

After using `python3 -m http.server 80` and wget from Celestial to move
the pspy64 binary to the /tmp dir, we'll run it. After spitting out a
bunch of errors and then info on all the running processes, it stops and
waits. When the 5 minute point comes around, here's what we see:
...
I'll look at the cron in the "Other Stuff" section at the end, but we
basically get all of it right here. Very cool!

### Hijack cron / Shell as root

So, hijack that file. Create a python rev shell one liner. Save the
original script so we can put it back when we're done, and put ours into
place:
...
The code for the server that was running is found in the sun homedir in
`server.js`:
...
We can see how if there's no cookie, it sets one and sends "404", and
how if there is, it uses `unserialize` and then reads values to make a
string.

### cron

As root, we can see the cron job that is running `script.py`, and then
copying the `script.py` from `/root` to reset it to it's original value:
...
The cron also sets the timestamps on `script.py` to match `user.txt`, so
that it's not obvious that it's being update regularly. To do this, it
uses `date -R -r`, which returns the modification time on `user.txt` in
RFC 5322 format. This is passed as an argument to `touch`, where the
`-d` flag allows us to set the timestamp instead of using the current
time.
