...
#malware #doc #vba #powershell #emotet #invoke-obfuscation Jun 4, 2018
...
Interesting sample from VT which ends up being a phishing document for
the Emotet malware.
...
There's a bunch of obfuscated VBA. Could probably use a debugger to
...
oHEwmITMTRZuEi. In the root document VBA, there's an Autoopen()
function. It has a bunch of garbage, but halfware through calls
`pNTOsHzwn (BhECP)`. Those are the other two functions in that section.
...
Opening in `Word` and putting a break at the interesting line in
`pT0sHzwn` reveals the encoded powershell.
...
similar sample, INV601213082839.doc, this works (even on powershell in
kali!):
...
`Windows Components` -\> `Windows PowerShell`, I turned on
`PowerShell Script Block Logging`.

![](/img/1528048315482.png)

Then I ran the base64 encoded command, and checked the logs in
`Event Viewer`, under `Applications and Services Logs` -\> `Microsoft`
-\> `Windows` -\> `PowerShell` -\> `Operational`. There were a bunch. It
looks like 4104 is the event type that can capture what is run.

First, there's a log with the encoded command as entered:

![](/img/1528048726641.png)

Then the decoded command that looks like the base64 we were able to
extract:

![](/img/1528048763286.png)

Then the deobfuscated command:

![](/img/1528048790325.png)
...
Turns out, after going back and playing around in powershell a bit more,
you can get the IEX string, with a print statement:
...
There's an error at the front, but if you cut after the `::`, you get
the powershell we're looking for.

## Summary / C2

It tries to download from each of the following, save it in public under
a random number .exe, and execute it:
...
([github](https://github.com/R3MRUM/PSDecode)), which looks really cool.
